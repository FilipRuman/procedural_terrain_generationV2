...
#nullable enable
		out WaterGen.LakeSpawningData? lake_spawning_data)
	{
		...
		{
			lake_spawning_data = new(lake_data.water_height);
		}
		...
		for (int x = -1; x < triangles_per_dimension + 1; x++)
		{
			for (int z = -1; z < triangles_per_dimension + 1; z++)
			{
				...
				{
					height = CalculateHeightLake(worldPos, lake_data, base_world_position, chunk_size);
					lake_spawning_data!.handle_new_vertex(new(worldPos.X, height, worldPos.Y));
				}
	...
	public float CalculateHeightLake(Vector2 world_position, WaterGen.LakeData lake_data, Vector2 chunk_base_pos, float chunk_size)
	{
		var base_height = CalculateHeight(world_position);
		Vector2 chunk_middle_point = chunk_base_pos + Vector2.One * (chunk_size / 2f);
		return base_height - lake_height_change_curve.SampleBaked(chunk_middle_point.DistanceSquaredTo(world_position));
	}
