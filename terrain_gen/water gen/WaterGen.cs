...
{
        [Export] public WaterGridRiverGen water_grid_river_gen;
        [Export] public MeshChunksRiverGen mesh_chunks_river_gen;
        ...
        public class OutputData(Dictionary<Vector2I, LakeData> world_pos_lakes, WaterGridRiverGen.RiverDataGrid river_grid, Vector2I test_world_base_pos)
        {
                public Dictionary<Vector2I, LakeData> world_pos_lakes = world_pos_lakes;
                public WaterGridRiverGen.RiverDataGrid river_grid = river_grid;
        ...
        public class WaterDataGrid
        {
                ...
                private readonly int mesh_triangles_per_dimension;
                ...
                public bool IsObjectUnderTheWater(Vector3 world_pos_f)
                {
                        ...
                        Vector3 pos_inside_a_mesh_chunk_f = world_pos_f - mesh_chunk_pos_f.Floor() * mesh_chunk_size;
                        Vector3 mesh_vertex_pos_f = pos_inside_a_mesh_chunk_f * (mesh_triangles_per_dimension / (float)mesh_chunk_size);
                        Vector2I mesh_vertex_pos = new(Mathf.FloorToInt(mesh_vertex_pos_f.X), Mathf.FloorToInt(mesh_vertex_pos_f.Z));

                        var water_chunk = this[world_pos];

                        if (water_chunk.world_pos_lakes.TryGetValue(mesh_chunk_world_pos, out var lake))
                        {
                                out_bool |= lake.water_height > world_pos_f.Y;
                        }

                        var data_grid = water_chunk.river_grid.AccessDataWithWorldPos(world_pos, mesh_chunk_size).mesh_chunk_data_grid;
                        if (data_grid != null)
                        {
                                out_bool |= data_grid.IsVertexInsideARiver(mesh_vertex_pos);
                        }
                        return out_bool;


                }
                public bool IsStructureCollidingWithARiver(StructureInstanceData structure)
                {
                        foreach (var world_pos in structure.MeshChunksThisStructureSitsOnWorldPos(mesh_chunk_size))
                        {
                                var water_chunk = this[world_pos];
                                var river_chunk = water_chunk.river_grid.AccessDataWithWorldPos(world_pos, mesh_chunk_size);
                                if (river_chunk.contains_river)
                                {
                                        return true;
                                }
                        }
                        ...
                        for (int x = 0; x < grid_width; x++)
                        {
                                for (int y = 0; y < grid_width; y++)
                                {
                                        ...
                                        if (new_x == 1 || new_y == 1)
                                        {
                                                ...
                                                grid[x + y * grid_width] = water_gen.GenerateCell(new(world_x, world_y), mesh_gen, mesh_chunk_size, mesh_triangles_per_dimension);
                                        }
                ...
                public WaterDataGrid(WaterGen water_gen, ThreadSafeGroundMeshGen mesh_gen, int mesh_triangles_per_dimension, int mesh_chunk_size, Vector2 player_world_pos)
                {
                        grid_cell_size = (int)water_gen.mesh_chunks_per_water_chunk * mesh_chunk_size;
                        this.mesh_triangles_per_dimension = mesh_triangles_per_dimension;
        ...
        public OutputData GenerateCell(Vector2I world_base_pos, ThreadSafeGroundMeshGen mesh_gen, int mesh_chunk_size, int mesh_triangles_per_dimension)
        {
                ...
                ChunkHeightGrid min_height_with_river_margins_grid = new(mesh_chunks_per_water_chunk);
                int river_margin_size = Mathf.CeilToInt(mesh_chunks_river_gen.Margin * mesh_chunk_size / (float)mesh_triangles_per_dimension) + 3;
                for (int x = 0; x < mesh_chunks_per_water_chunk; x++)
                {
                        for (int y = 0; y < mesh_chunks_per_water_chunk; y++)
                        {
                                ...
                                GetChunkHeightStats(river_margin_size, mesh_chunk_size, chunk_base_pos, mesh_gen, out float average_height, out float min_height, out float min_height_with_river_margins);
                                min_height_with_river_margins_grid[x, y] = min_height_with_river_margins;
                ...
                WaterGridRiverGen.RiverGoalsData[] rivers;
                if (lakes.Count != 0)
                {
                        rivers = new WaterGridRiverGen.RiverGoalsData[river_start_points.Count];
                        for (int i = 0; i < river_start_points.Count; i++)
                        {
                                ...
                                if (min_distance_lake == null) { break; }
                ...
                var river_data = water_grid_river_gen.GenerateDataForWaterChunk(average_height_grid, rivers, (int)mesh_chunks_per_water_chunk, world_base_pos, mesh_chunk_size);
        ...
        private void AddChunkToLakesOrRivers(float average_height, Vector2I chunk, ref List<LakeData> lakes, ref List<Vector2I> river_start_points)
        {
                if (lake_gen.IsChunkLake(average_height))
                        lakes.Add(new(chunk));
                if (water_grid_river_gen.DoesChunkContainRiverStart(average_height))
                        river_start_points.Add(chunk);
        }

        public void GetChunkHeightStats(int river_margin_size, int mesh_chunk_size, Vector2 chunk_base_world_pos, ThreadSafeGroundMeshGen mesh_gen,
                                        out float average_height, out float min_height, out float min_height_with_river_margins)
        {
                var sum = 0f;
                min_height = float.MaxValue;
                var distance_per_check = mesh_chunk_size / (float)height_checks_per_chunk_sqrt;
                min_height_with_river_margins = float.MaxValue;
                for (int x = -1; x < height_checks_per_chunk_sqrt + 1; x++)
                {
                        for (int y = -1; y < height_checks_per_chunk_sqrt + 1; y++)
                        {
                                ...
                                if (x * distance_per_check > river_margin_size && y * distance_per_check > river_margin_size)
                                        min_height_with_river_margins = Mathf.Min(min_height_with_river_margins, height);
                        }
                }
